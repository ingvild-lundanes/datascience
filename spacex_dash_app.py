...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        filtered_df.columns = ['Launch Site', 'Count']
        ...
        title=f"Success ratio for {entered_site}")
        return fig
...
def get_scatterplot(entered_site, chosen_range):
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= chosen_range[0]) & (spacex_df['Payload Mass (kg)'] <= chosen_range[1])]
    if entered_site == 'ALL':
